...
# Load your MIDI file
midi_file_path = 'path_to_your_midi_file.mid'  # Replace with the actual path to your MIDI file
...
for msg in midi.play():
    if msg.type == 'note_on' and msg.velocity > 0:  # Note on event with non-zero velocity
        note_number = msg.note
        note_name = note_names.get(note_number, None)
        if note_name:
            print(f"Playing note: {note_name}")
    # Simulate waiting time to match real-time note playback
    time.sleep(msg.time)