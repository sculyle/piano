...
# Load your MIDI file
midi_file_path = '/home/instrumentgroup/Downloads/Pathetique.mid' # Replace with your actual path
midi = mido.MidiFile(midi_file_path)

# Define a dictionary to map MIDI note numbers to note names
note_names = {
    21: 'A0', 22: 'A#0', 23: 'B0',
...
# Initialize tempo and time
tempo = 500000  # Default to 120 BPM if no tempo is specified
...
def ticks_to_seconds(ticks, tempo, ticks_per_beat):
    ...
    return (ticks * microseconds_per_tick) / 1_000_000  # Convert microseconds to seconds
...
for track in midi.tracks:
    for msg in track:
        if msg.type == 'set_tempo':
            ...
            time_elapsed += ticks_to_seconds(msg.time, microseconds_per_beat, ticks_per_beat)
            note_number = msg.note
            note_name = note_names.get(note_number, None)
            if note_name:
                print(f"Playing note: {note_name} at {time_elapsed:.2f} seconds")
        else:
            # Handle other message types like 'note_off' or 'control_change' if needed
            time_elapsed += ticks_to_seconds(msg.time, microseconds_per_beat, ticks_per_beat)
