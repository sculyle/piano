...
def bar(): # <- Function that will update value of duration bar
    ...
    if progress_scale['value'] < music_length and stop == 0 and enable>0:
        if (oldenable == enable):
...
pause_button.place(x=175, y=360)
...
velocity_slider_label = tk.Label(root, text="Set Velocity Filter", font=('Ariel', 11), background='white')
#velocity_slider_label.grid(columnspan=6, column=10, row=19)
velocity_slider_label.place(x=550, y=370)

velocity_slider = tk.Scale(root, from_=0, to=100, orient=tk.HORIZONTAL, variable=velocity_percentage, command=slider_update)
#velocity_slider.grid(column=10, row=16)
velocity_slider.place(x=560, y=390)
...
#channels_checkbox.grid(columnspan=2, column=10, row=20)
channels_checkbox.place(x=560, y=440)
...
def display():
    ...
    canvas.create_image(180, 10, image = canvas.image_tk, anchor = 'nw')
    label2.config(text=song)
    label2.grid(columnspan =3, column = 9, row = 14)
